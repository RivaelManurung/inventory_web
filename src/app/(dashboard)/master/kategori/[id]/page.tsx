...
export default async function DetailKategoriPage({
  ...
  const data = await prisma.barangCategory.findUnique({
    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <DeleteAction id={data.id} name={data.name} module="kategori" />
          <Link
            href={`/master/kategori/${data.id}/edit`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-foreground">{data._count.barangs} item</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          ...
          <div className="p-6">
            ...
            <p className="text-xs text-muted-foreground mt-1">item dalam kategori ini</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="px-6 py-4 border-b border-border bg-muted/30 flex items-center gap-2">
          ...
          <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-[0.2em]">
            Barang Terbaru di Kategori Ini
          </h2>
        </div>
            ...
            <tbody className="divide-y divide-border">
              {data.barangs.map((b) => (
                <tr key={b.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-4 text-sm font-medium text-foreground text-right pr-6">
                    Rp {Number(b.barangHarga).toLocaleString("id-ID")}
                  </td>
                </tr>
              ))}
