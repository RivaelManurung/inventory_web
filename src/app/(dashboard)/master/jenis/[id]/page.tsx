...
export default async function DetailJenisPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const data = await prisma.jenisBarang.findUnique({
    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/master/jenis" className="p-2 bg-card border border-border text-muted-foreground rounded-md hover:bg-accent transition-colors">
        ...
        <div className="flex items-center gap-2">
          <DeleteAction id={data.id} name={data.name} module="jenis" />
          <Link href={`/master/jenis/${data.id}/edit`} className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-md text-sm font-medium hover:bg-primary/90 transition-all font-semibold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          <div className="px-5 py-4 border-b border-border bg-muted/30">
            <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-[0.2em]">Total Asset</p>
          </div>
          <div className="px-5 py-4">
            <p className="text-3xl font-bold text-foreground">{data._count.barangs}</p>
            <p className="text-xs text-muted-foreground mt-1">barang terkait</p>
          </div>
        </div>
      </div>
