...
export default function EditJenisPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/master/jenis/${id}`, {
        method: "PATCH",
      ...
      if (resData.success) {
        toast.success("Perubahan jenis barang berhasil disimpan!", { id: toastId });
        router.push(`/master/jenis/${id}`);
        router.refresh();
      } else {
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex items-center gap-3">
        <Link href={`/master/jenis/${id}`} className="p-2 bg-card border border-border text-muted-foreground rounded-md hover:bg-accent transition-colors"><ArrowLeft className="w-4 h-4" /></Link>
        <div>
          <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">Master Data / Jenis / {data.name}</p>
          <h1 className="text-lg font-semibold text-foreground">Edit Jenis Barang</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-xl border border-border bg-card overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Nama Jenis Barang <span className="text-destructive">*</span></label>
              <input 
                type="text" 
                value={form.name}
            ...
            <div className="space-y-1.5">
              ...
              <textarea 
                ...
                placeholder="Jelaskan karakteristik jenis barang ini..."
            ...
            <div className="pt-4 border-t border-border flex items-center justify-between">
              <Link href={`/master/jenis/${id}`} className="px-4 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors">Batal</Link>
              <button 
                type="submit" 
                disabled={saving}
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden h-fit">
          <div className="px-6 py-4 border-b border-border bg-muted/30 flex items-center gap-2">
            ...
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-[0.2em]">Info Jenis</h2>
          </div>
