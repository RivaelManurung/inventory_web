...
import { Plus, Search, Filter, Download, Layers, ChevronRight, Archive, Edit2, Trash2 } from "lucide-react";
...
export default async function JenisBarangPage({
  ...
  const totalItems = jenisBarang.reduce((acc, curr) => acc + curr._count.barangs, 0);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Jenis Barang</h1>
          <p className="text-slate-500 text-sm font-medium">Kelola klasifikasi atau jenis barang untuk mempermudah inventarisasi.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link href="/master/jenis/create" className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-500 transition-all text-xs font-black uppercase tracking-widest shadow-lg shadow-blue-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between group cursor-default">
              <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Total Jenis</span>
              </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/30">
          <form className="relative w-full md:w-96" action="/master/jenis" method="GET">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Jenis Barang</th>
        ...
        <div className="p-6 border-t border-slate-50 bg-slate-50/20 flex justify-between items-center">
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Menampilkan {jenisBarang.length} hasil</p>
        </div>
      </div>
    </div>
  );
