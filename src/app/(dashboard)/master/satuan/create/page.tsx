...
export default function CreateSatuanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const toastId = toast.loading("Sedang menyimpan satuan...");
    try {
      const res = await fetch("/api/master/satuan", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      const data = await res.json();
      if (data.success) { 
        toast.success("Satuan berhasil disimpan!", { id: toastId });
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      <HeaderTitle title="Tambah Satuan" />
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="text-lg font-semibold text-foreground">Tambah Satuan Baru</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-xl border border-border bg-card overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Nama Satuan (Pcs, Box, Liter, dll) <span className="text-destructive">*</span></label>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Identitas Sistem (Otomatis)</label>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Keterangan Singkat</label>
            ...
            <div className="pt-4 border-t border-border flex items-center justify-between">
              <Link href="/master/satuan" className="px-4 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors">Batal</Link>
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden h-fit">
          ...
          <div className="p-5 space-y-3">
            <p className="text-xs text-muted-foreground leading-relaxed">Satuan mendefinisikan unit pengukuran untuk setiap item barang dalam sistem.</p>
            <ul className="space-y-2 text-xs text-muted-foreground">
              <li className="flex items-start gap-2"><div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5 shrink-0" />Contoh: Pcs, Box, Kg, Liter</li>
              <li className="flex items-start gap-2"><div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5 shrink-0" />Slug dibuat otomatis dari nama</li>
              <li className="flex items-start gap-2"><div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5 shrink-0" />Satuan digunakan saat menambahkan barang</li>
            </ul>
          </div>
        </div>
      </div>
