  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert("Gagal menambahkan gudang: " + data.message);
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-700 max-w-3xl">
      <div className="flex items-center gap-4">
        <Link href="/master/gudang" className="p-2.5 bg-white border border-slate-200 text-slate-500 rounded-xl hover:bg-slate-50 transition-all shadow-sm">
        ...
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Tambah Gudang / Lokasi</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden p-8">
        <div className="space-y-6">
          <div>
            <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Nama Gudang / Lokasi</label>
            <input 
              type="text" 
              value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              placeholder="Contoh: Gudang Utama, Ruang Server Lt 2"
          ...
          <div>
            ...
            <textarea 
              ...
              placeholder="Detail alamat atau keterangan gudang..."
              rows={3}
          ...
          <div className="pt-6 border-t border-slate-100 flex justify-end">
            ...
            >
              {loading ? "Menyimpan..." : <><Save className="w-4 h-4" /> Simpan Gudang</>}
            </button>
          </div>
        </div>
      </form>
