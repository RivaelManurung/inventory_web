...
import { Search, MapPin, Filter, ArrowUpDown, Plus, Box, Calendar, ChevronLeft, ChevronRight } from "lucide-react";
...
export default async function GudangPage({
  ...
  const [gudangList, totalItems] = await Promise.all([
    ...
    <div className="space-y-4 animate-in fade-in duration-500 font-inter">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/master/gudang/create" className="h-9 px-4 bg-primary text-primary-foreground rounded-md text-sm font-medium hover:bg-primary/90 transition-all flex items-center gap-1.5 shadow-sm flex-none">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/30 border-b border-border">
                <th className="pl-6 pr-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-[0.2em] w-[30%]">
                  <div className="flex items-center gap-1.5">
                    Gudang / Lokasi <ArrowUpDown className="h-3.5 w-3.5 text-muted-foreground/50" />
                  </div>
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-[0.2em] w-[30%]">Deskripsi</th>
                <th className="px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-[0.2em] w-[25%]">Kapasitas & Info</th>
              ...
              {gudangList.length > 0 ? (
                gudangList.map((item) => (
                  <tr key={item.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-4 pr-6 text-sm font-normal text-muted-foreground">
                      {item.description || "-"}
                    </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-2 py-4">
          ...
          <div className="flex gap-2">
            <Link
              href={`/master/gudang?q=${search}&page=${currentPage - 1}`}
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <Link
                  key={p}
                  href={`/master/gudang?q=${search}&page=${p}`}
