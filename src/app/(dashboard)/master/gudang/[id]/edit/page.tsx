  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex items-center gap-3">
        <Link href={`/master/gudang/${id}`} className="p-2 bg-card border border-border text-muted-foreground rounded-md hover:bg-accent transition-colors"><ArrowLeft className="w-4 h-4" /></Link>
        <div>
          <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">Master Data / Gudang / {data.name}</p>
          <h1 className="text-lg font-semibold text-foreground">Edit Lokasi Gudang</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-xl border border-border bg-card overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Nama Gudang / Cabang <span className="text-destructive">*</span></label>
              <input 
                type="text" 
                value={form.name}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Slug Identitas (URL)</label>
            ...
            <div className="pt-4 border-t border-border flex items-center justify-between">
              <Link href={`/master/gudang/${id}`} className="px-4 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors">Batal</Link>
              <button 
                type="submit" 
                disabled={saving}
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden h-fit">
          <div className="px-6 py-4 border-b border-border bg-muted/30 flex items-center gap-2">
            ...
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-[0.2em]">Info Gudang</h2>
          </div>
          <div className="p-5 space-y-4">
            <div>
              ...
              <div className="inline-flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-emerald-600 uppercase">Gudang Aktif</span>
              </div>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed border-t border-border pt-3">
              Perubahan alamat akan langsung tercermin pada semua dokumen pengiriman barang ke lokasi ini.
            </p>
          </div>
        </div>
      </div>
